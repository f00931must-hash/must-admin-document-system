...
onAuthStateChanged(auth,async user=>{currentUser=user;currentAccess=null;$("appView").classList.add('hidden');$("loginView").classList.add('hidden');$("deniedView").classList.add('hidden');if(!user){$("loginView").classList.remove('hidden');return}try{const email=normalizedEmail(user.email),[settingsSnap,assistantSnap]=await Promise.all([getDoc(doc(db,'settings','adminAccess')),getDoc(doc(db,'administrativeAssistants',email))]),access=assistantSnap.exists()?assistantSnap.data():settingsSnap.data()?.users?.[email];if(!access||access.enabled===false)throw new Error('not-authorized');currentAccess=access;$("appView").classList.remove('hidden');$("userEmail").textContent=`${access.displayName||email}\n${email}`;}catch(err){console.error(err);$("deniedMessage").textContent='此帳號尚未由資源教室行政平台開通行政文書權限，或權限尚未同步。';$("deniedView").classList.remove('hidden');}});

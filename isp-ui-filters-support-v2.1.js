...
function ensureTotalFilters(){const head=$("mine")?.querySelector(".page-head");if(!head||$("ispClassFilter"))return;const wrap=document.createElement("div");wrap.className="sort-control";wrap.style.cssText="display:flex;gap:8px;align-items:end;flex-wrap:wrap";wrap.innerHTML='<label>系級篩選<select id="ispClassFilter"><option value="">全部系級</option></select></label><label>年級篩選<select id="ispGradeFilter"><option value="">全部年級</option></select></label>';head.insertBefore(wrap,head.querySelector(".sort-control")||null);wrap.querySelectorAll("select").forEach(s=>s.addEventListener("change",applyTotalFilters));}
function applyTotalFilters(){ensureTotalFilters();const list=$("docList");if(!list)return;const data=docs.filter(d=>!d.type||d.type==="ISP"),classSel=$("ispClassFilter"),gradeSel=$("ispGradeFilter"),oldC=classSel?.value||"",oldG=gradeSel?.value||"";const classes=[...new Set(data.map(docClass).filter(x=>x!=="未設定"))].sort((a,b)=>a.localeCompare(b,"zh-Hant"));const grades=[...new Set(data.map(docGrade).filter(x=>x!=="未設定"))].sort((a,b)=>(gradeOrder[a]??99)-(gradeOrder[b]??99));if(classSel){classSel.innerHTML=`<option value="">全部系級</option>${classes.map(x=>`<option value="${x}">${x}</option>`).join("")}`;if(classes.includes(oldC))classSel.value=oldC;}if(gradeSel){gradeSel.innerHTML=`<option value="">全部年級</option>${grades.map(x=>`<option value="${x}">${x}年級</option>`).join("")}`;if(grades.includes(oldG))gradeSel.value=oldG;}const c=classSel?.value||"",g=gradeSel?.value||"";[...list.children].filter(n=>n.classList.contains("doc-item")).forEach(node=>{const d=totalDocForNode(node);if(!d)return;node.dataset.filterClass=docClass(d);node.dataset.filterGrade=docGrade(d);node.style.display=(!c||node.dataset.filterClass===c)&&(!g||node.dataset.filterGrade===g)?"flex":"none";});}
...
function ensureSemesterFilters(){const head=$("semesterIsp")?.querySelector(".page-head");if(!head||$("semesterClassFilter"))return;const wrap=document.createElement("div");wrap.className="sort-control";wrap.style.cssText="display:flex;gap:8px;align-items:end;flex-wrap:wrap";wrap.innerHTML='<label>系級篩選<select id="semesterClassFilter"><option value="">全部系級</option></select></label><label>年級篩選<select id="semesterGradeFilter"><option value="">全部年級</option></select></label>';const sort=$("semesterIspSort")?.parentElement;head.insertBefore(wrap,sort||$("newSemesterIspListBtn"));wrap.querySelectorAll("select").forEach(s=>s.addEventListener("change",applySemesterFilters));}
function applySemesterFilters(){ensureSemesterFilters();const list=$("semesterIspList");if(!list)return;const data=docs.filter(d=>d.type==="SEMESTER_ISP"),classSel=$("semesterClassFilter"),gradeSel=$("semesterGradeFilter"),oldC=classSel?.value||"",oldG=gradeSel?.value||"";const classes=[...new Set(data.map(docClass).filter(x=>x!=="未設定"))].sort((a,b)=>a.localeCompare(b,"zh-Hant"));const grades=[...new Set(data.map(docGrade).filter(x=>x!=="未設定"))].sort((a,b)=>(gradeOrder[a]??99)-(gradeOrder[b]??99));if(classSel){classSel.innerHTML=`<option value="">全部系級</option>${classes.map(x=>`<option value="${x}">${x}</option>`).join("")}`;if(classes.includes(oldC))classSel.value=oldC;}if(gradeSel){gradeSel.innerHTML=`<option value="">全部年級</option>${grades.map(x=>`<option value="${x}">${x}年級</option>`).join("")}`;if(grades.includes(oldG))gradeSel.value=oldG;}const c=classSel?.value||"",g=gradeSel?.value||"";const groups=[...list.querySelectorAll(":scope > .semester-student-group")];for(const group of groups){const records=[...group.querySelectorAll(".semester-grade-records > .doc-item")],matches=[];for(const node of records){const d=semesterDocFromRecordNode(node);if(!d)continue;matches.push({node,d,cls:docClass(d),grade:docGrade(d)});}const ok=matches.some(x=>(!c||x.cls===c)&&(!g||x.grade===g));group.style.display=ok?"block":"none";if(ok&&g){const picker=group.querySelector(".semester-grade-picker");if(picker&&[...picker.options].some(o=>o.value===g)){picker.value=g;picker.dispatchEvent(new Event("change"));}}}
  if(!groups.length){[...list.children].filter(n=>n.classList.contains("doc-item")).forEach(node=>{const d=semesterDocFromRecordNode(node);if(!d)return;node.style.display=(!c||docClass(d)===c)&&(!g||docGrade(d)===g)?"flex":"none";});}}
...
function applyAll(){ensureSemesterSupportFields();ensureTotalFilters();ensureSemesterFilters();applyTotalFilters();applySemesterFilters();}
...
ensureSemesterSupportFields();ensureTotalFilters();ensureSemesterFilters();
onAuthStateChanged(auth,async user=>{ownerEmail=await resolveOwner(user);if(user)await refreshDocs();});
console.log("ISP UI filters/support v2.1.0 loaded");

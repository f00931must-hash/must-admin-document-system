...
$("downloadTeacherSummaryBtn").onclick=async()=>{
  try{
    ...
    const renderData={department:$("teacherSummaryDepartment").value.trim(),studentClass:$("teacherSummaryClass").value.trim(),studentName:$("teacherSummaryStudentName").value.trim(),disabilityType:$("teacherSummaryDisability").value.trim(),advisorName:$("teacherSummaryAdvisor").value.trim().replace(/老師$/,"")};
    for(let i=1;i<=7;i++)renderData[`status${i}`]=statusItems[i-1]?[{text:statusItems[i-1]}]:[];
    for(let i=1;i<=6;i++)renderData[`strategy${i}`]=strategyItems[i-1]?[{text:strategyItems[i-1]}]:[];

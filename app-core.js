...
function normalizedStudentName(value){return String(value||"").trim().replace(/[\s　]+/g,"");}
function duplicateReasonFor(docItem){
  const name=normalizedStudentName(docItem.studentName||docItem.form?.studentName);
  if(!name)return "";
  const id=String(docItem.studentId||docItem.form?.studentId||"").trim();
  const year=academicYearFromAdmission(docItem.form?.admissionDate);
  const matches=ispDocuments.filter(other=>{
    if(other.id===docItem.id)return false;
    if(normalizedStudentName(other.studentName||other.form?.studentName)!==name)return false;
    const otherId=String(other.studentId||other.form?.studentId||"").trim();
    const otherYear=academicYearFromAdmission(other.form?.admissionDate);
    if(id&&otherId&&id===otherId)return true;
    if(year&&otherYear&&year===otherYear)return true;
    return false;
  });
  if(!matches.length)return "";
  const sameId=matches.some(other=>{
    const otherId=String(other.studentId||other.form?.studentId||"").trim();
    return id&&otherId&&id===otherId;
  });
  return sameId?"同姓名＋同學號":"同姓名＋同學年度";
}
...
function renderDocs(){
  ...
  const items=sortedIspDocuments();
  if(!items.length){list.innerHTML='<div class="doc-item">目前尚無新生 ISP 總表。</div>';return;}
  for(const d of items){
    ...
    if(deleteButton)deleteButton.onclick=async()=>{
      ...
      if(!confirm(`確定要從目前列表移除「${name}」的新生 ISP 總表嗎？\n\n系統會先保留安全備份，再從列表移除。`))return;
      if(!confirm(`請再次確認：要移除「${name}」嗎？`))return;
      ...
      try{
        ...
        await addDoc(collection(db,'adminDocuments'),{
          ...
          createdByEmail:normalizedEmail(currentUser.email)
        });
        await deleteDoc(doc(db,'adminDocuments',d.id));
        window.__adminDocumentsCache?.invalidate?.(workspaceOwnerEmail());
        ispDocuments=ispDocuments.filter(item=>item.id!==d.id);
        renderDocs();
        alert('已從目前列表刪除；系統已先保留一份安全備份。');
      }catch(error){
        console.error(error);
        deleteButton.disabled=false;
        alert('刪除失敗，請確認帳號權限或稍後再試。');
      }
    };
...
const semesterStrengths=[
  ["strengthRelationship","建立人際關係能力"],["strengthEmotion","情緒控制能力"],
  ["strengthIllnessAwareness","個人疾病認識能力"],["strengthProblemSolving","解決問題及處理狀況能力"],
  ["strengthResourceSeeking","尋求資源能力"],["strengthSupportSystem","支持系統資源"],
  ["strengthFamilyInteraction","家人的互動與關懷"],["strengthFamilyEconomy","家庭經濟狀況"]
];
const semesterAnalyses=[
...
function aiFieldLabel(name,form=$("ispForm")){
...
function buildAiSource(mode,form=$("ispForm")){
  const values=serializeForm(form);
  const formatFields=fields=>fields.map(name=>{
    const value=values[name];
    const text=Array.isArray(value)?value.filter(Boolean).join("、"):String(value||"").trim();
    return text?`${aiFieldLabel(name,form)}：${text}`:"";
  }).filter(Boolean).join("\n");
...
document.querySelectorAll(".ai-generate-btn").forEach(button=>{
  ...
  button.addEventListener("click",async()=>{
    ...
    const source=buildAiSource(button.dataset.aiMode,form);
...
$("downloadSemesterIspBtn").onclick=async()=>{
  try{
    ...
    const zip=new window.PizZip(await res.arrayBuffer());
    const docx=new window.docxtemplater(zip,{paragraphLoop:true,linebreaks:true,nullGetter:()=>""});
    docx.render(semesterExportData(f));
    const blob=docx.getZip().generate({type:"blob",mimeType:"application/vnd.openxmlformats-officedocument.wordprocessingml.document"});
    const safe=(f.studentName||"未命名").replace(/[\\/:*?"<>|]/g,"_");
...
const timetableModeConfig={
  general:{days:[1,2,3,4,5,6],labels:["星期一","星期二","星期三","星期四","星期五","星期六"]},
  weekend:{days:[6,7],labels:["星期六","星期日"]}
};

function parseTimetableHtml(html,mode="general"){
  if(!html)throw new Error("沒有取得網站表格格式，請直接從網站按 Ctrl+A、Ctrl+C 後貼上。");
  const config=timetableModeConfig[mode]||timetableModeConfig.general;
  const holder=document.createElement("div");
  holder.innerHTML=html;
  const tables=[...holder.querySelectorAll("table")];
  const table=tables.find(item=>config.labels.every(day=>(item.textContent||"").includes(day)));
  if(!table)throw new Error(mode==="weekend"?"找不到包含星期六、星期日的課表，請確認已複製完整頁面。":"找不到課表。請確認貼上的是『本學期課表查詢』完整頁面。");
  ...
  const header=rows.find(row=>config.labels.every(day=>(row.textContent||"").includes(day)));
  if(!header)throw new Error("課表星期欄位無法辨識。");
  const headers=[...header.cells].map(cell=>(cell.textContent||"").replace(/\s+/g,""));
  const dayIndexes=config.labels.map(day=>headers.findIndex(text=>text.includes(day)));
  if(dayIndexes.some(index=>index<0))throw new Error(`課表缺少${config.labels.join("、")}欄位，請重新複製完整頁面。`);
  const courses={};
  timetablePeriods.forEach(item=>{for(let day=1;day<=7;day++)courses[`p${item.period}d${day}`]=""});
  for(const row of rows){
    ...
    dayIndexes.forEach((cellIndex,dayOffset)=>{
      const dayNumber=config.days[dayOffset];
      courses[`p${period.period}d${dayNumber}`]=cleanTimetableCourse(row.cells[cellIndex]);
    });
  }
  return {
...
function renderTimetablePreview(data){
  ...
  const config=timetableModeConfig[data.timetableMode]||timetableModeConfig.general;
  $("timetablePreviewHead").innerHTML=`<tr><th>節次／時間</th>${config.labels.map(label=>`<th>${label}</th>`).join("")}</tr>`;
  const body=$("timetablePreviewBody");body.innerHTML="";
  const visiblePeriods=timetablePeriods.filter(item=>config.days.some(day=>data.courses[`p${item.period}d${day}`]));
  for(const item of visiblePeriods){
    ...
    for(const day of config.days){const cell=document.createElement("td");cell.textContent=data.courses[`p${item.period}d${day}`]||"";row.appendChild(cell);}
...
function runTimetableParsing(){
  ...
  try{
    parsedTimetable=parseTimetableHtml(timetableClipboardHtml,$("timetableType").value);
...
$("timetableType").addEventListener("change",()=>{
  parsedTimetable=null;
  $("timetablePreview").classList.add("hidden");
  $("timetableStatus").textContent="課表類型已變更，請重新按「格式優化」。";
  $("timetableStatus").classList.remove("error");
});
...
$("downloadTimetableBtn").onclick=async()=>{
  ...
  try{
    const weekendMode=parsedTimetable.timetableMode==="weekend";
    const template=weekendMode?"timetable-weekend-template.docx":"timetable-template.docx";
    const response=await fetch(`./templates/${template}?v=1.4.0`,{cache:"no-store"});
    ...
    const outputDays=weekendMode?[6,7]:[1,2,3,4,5,6];
    for(let period=1;period<=15;period++)periodVisibility[`show${period}`]=outputDays.some(day=>parsedTimetable.courses[`p${period}d${day}`]);

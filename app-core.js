...
$("ispForm").onsubmit=async e=>{e.preventDefault();if(!currentUser||!currentAccess)return;const form=formData(),ownerEmail=workspaceOwnerEmail();const common={ownerEmail,type:'ISP',studentName:(form.studentName||'').trim(),studentId:(form.studentId||'').trim(),form,updatedAt:serverTimestamp(),lastEditorUid:currentUser.uid,lastEditorEmail:normalizedEmail(currentUser.email)};const id=$("docId").value;try{document.dispatchEvent(new CustomEvent("isp:manual-save-start"));if(id)await updateDoc(doc(db,'adminDocuments',id),common);else{const payload={...common,ownerUid:currentUser.uid,createdByUid:currentUser.uid,createdByEmail:normalizedEmail(currentUser.email),createdAt:serverTimestamp()};const ref=await addDoc(collection(db,'adminDocuments'),payload);$("docId").value=ref.id;}document.dispatchEvent(new CustomEvent("isp:manual-save-success"));alert('草稿已儲存');}catch(error){console.error("ISP manual save failed",error);document.dispatchEvent(new CustomEvent("isp:manual-save-error"));alert('儲存失敗，資料仍保留在目前畫面，請確認網路後再試。');}};
let ispDocuments=[];
...
}catch(error){console.error(error);deleteButton.disabled=false;alert('刪除失敗，請確認帳號權限或稍後再試。');}};list.appendChild(div);}}
